...
/* 定义GPIO端口  PG7 = PS2_CLK   PG3 = PS2_DATA */
...
void bsp_InitPS2(void)
{
	...
	/* 配置CLK 和 DATA 为开漏输出引脚 */	
	gpio_init.Mode = GPIO_MODE_OUTPUT_OD;	/* 设置推挽输出 */
...
{
	/* 配置 PS2_CLK 作为中断输入口，下降沿触发 */
	{
		GPIO_InitTypeDef   GPIO_InitStructure;
		
		RCC_PS2_CLK_ENABLE();		/* 打开GPIO时钟 */
...
//		RCC_PS2_CLK_ENABLE();		/* 打开GPIO时钟 */
		...
		GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD;	/* 无中断 */;
		GPIO_InitStructure.Pull = GPIO_NOPULL;
		GPIO_InitStructure.Pin = PIN_PS2_CLK;
		HAL_GPIO_Init(PORT_PS2_CLK, &GPIO_InitStructure);	
