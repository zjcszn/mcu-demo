...
static void httpd_cookie_destory_head(void) {
    ...
    if (del_cookie != NULL) {
        ...
        cookie_list.count--;
        LWIP_DEBUGF(COOKIES_DEBUG, ("cur_cookie[%s]: destory, left:%u\r\n", del_cookie->string, cookie_list.count));
        mem_free(del_cookie);
    }
        ...
        for (target = &cookie_list.head; *target != NULL; target = &(*target)->next) {
            if (memcmp((*target)->string, cookie_string, LWIP_HTTPD_AUTH_COOKIE_LENGTH) == 0) {
                ...
                *target = del_cookie->next;
                cookie_list.count--;
                LWIP_DEBUGF(COOKIES_DEBUG,
                            ("cur_cookie[%s]: destory, left:%u\r\n", del_cookie->string, cookie_list.count));
                mem_free(del_cookie);
                ret = ERR_OK;
                break;
            }
...
static void httpd_cookie_insert(struct http_cookie *new_cookie) {
    ...
    cookie_list.count++;
    LWIP_DEBUGF(COOKIES_DEBUG, ("cur_cookie[%s]: insert , left:%u\r\n", new_cookie->string, cookie_list.count));
    sys_mutex_unlock(&cookie_list.mux);
}
...
const char *httpd_cookie_generate(void) {
    ...
    LWIP_ASSERT("can't create new http cur_cookie, no memory", new_cookie != NULL);
...
void httpd_cookie_init(void) {
    ...
    ret = sys_timer_new(&cookie_list.timer, "cur_cookie timer", 1, httpd_cookie_timer);
    LWIP_ASSERT("can't create cur_cookie timer", ret == ERR_OK);
}
...
int httpd_authorized_for_uri(void *connection, const char *uri, u32_t uri_len) {
    if (httpd_get_login_status(connection) == 1) {
