...
#设置用户变量
set(LTO TRUE CACHE BOOL "lto switch")
...
#设置 warnning 屏蔽列表
set(WARNNING_MASK "-Wno-unused-variable -Wno-knr-promoted-parameter" CACHE STRING "disable warnning list")

#设置debug/release编译参数
set(CMAKE_C_FLAGS_DEBUG "${WARNNING_MASK} -O0 -Wall" CACHE STRING "c debug flag" FORCE)
set(CMAKE_CXX_FLAGS_DEBUG ${CMAKE_C_FLAGS_DEBUG} CACHE STRING "c++ debug flag" FORCE)

set(CMAKE_C_FLAGS_RELEASE "${WARNNING_MASK} -O3 -Wall -DNDEBUG" CACHE STRING "c release flag" FORCE)
set(CMAKE_CXX_FLAGS_RELEASE ${CMAKE_C_FLAGS_RELEASE} CACHE STRING "c++ release flag" FORCE)

#add_link_options(--diag_suppress=L6304W)
...
#接口: LTO使能控制
function(CMSIS_SetLTO isEnabled)
    set(LTO ${isEnabled} CACHE BOOL "lto switch" FORCE)
endfunction()
...
function(CMSIS_CrossCompilingConfiguration)
    ...
    if (LTO)
        set(COMPILE_LTO_FLAG "-flto")
        set(LINK_LTO_FLAG "--lto")
    else ()
        set(COMPILE_LTO_FLAG "-fno-lto")
        set(LINK_LTO_FLAG "--no-lto")
    endif ()
    
    #设置C/C++通用编译选项
    set(COMPILE_COMMON_FLAG "\
        ${C_TARGET_FLAG} ${FPU_FLAG} ${COMPILE_LTO_FLAG} ${C_COMPILE_PREDEFINES} \
