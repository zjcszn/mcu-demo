...
void spi_master_init(void) {
    ...
    configASSERT(spi_sem != NULL);
    spi_mux = xSemaphoreCreateMutex();
    configASSERT(spi_mux != NULL);
}
 ...
 */
static void spi_nvic_init(void) {
    NVIC_SetPriority(SPI1_RX_DMA_IRQN, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0x7, 0));     // 使能SPI RX.DMA中断
    NVIC_EnableIRQ(SPI1_RX_DMA_IRQN);
    NVIC_SetPriority(SPI1_TX_DMA_IRQN, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0x7, 0));     // 使能SPI TX.DMA中断
