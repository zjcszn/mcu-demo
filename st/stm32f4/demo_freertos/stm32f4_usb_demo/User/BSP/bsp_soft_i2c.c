...
#if SOFT_I2C_ASSERT_EN
#define   SOFT_I2C_ASSERT(exp)        assert(exp)
#else
#define   SOFT_I2C_ASSERT(exp)        ((void)0U)
#endif

#if SOFT_I2C_DEBUG_EN
#define   SOFT_I2C_DEBUG(...)         log_i(__VA_ARGS__)
#else
#define   SOFT_I2C_DEBUG(...)         ((void)0U)
#endif

#if SOFT_I2C_ERROR_EN
#define   SOFT_I2C_ERROR(...)         log_e(__VA_ARGS__)
#else
#define   SOFT_I2C_ERROR(...)         ((void)0U)
#endif
...
void soft_i2c_init(soft_i2c_dev_t *i2c_dev, uint16_t i2c_freq) {
    ...
    GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;                       // set gpio no pull
    ...
    SOFT_I2C_DEBUG("soft i2c freq: %u Khz", 500 / i2c_dev->ck_delay);
    SOFT_I2C_DEBUG("soft i2c initialize success");
}
...
int soft_i2c_master_write(soft_i2c_dev_t *i2c_dev, const uint8_t *wr_data, uint16_t data_len) {
    ...
    if (soft_i2c_master_send_byte(i2c_dev, i2c_dev->dev_addr << 1)) {
        SOFT_I2C_ERROR("soft i2c error -> WR addr 0x%02X nack", i2c_dev->dev_addr);
 ...
 * @return int 0 on success | others on failed
 */
int soft_i2c_master_mem_write(soft_i2c_dev_t *i2c_dev, uint32_t mem_addr, uint16_t addr_len, const uint8_t *wr_data,
                              uint16_t data_len) {
...
int soft_i2c_master_read(soft_i2c_dev_t *i2c_dev, uint8_t *rd_data, uint16_t data_len) {
    ...
    if (soft_i2c_master_send_byte(i2c_dev, (i2c_dev->dev_addr << 1) | 1)) {
        SOFT_I2C_ERROR("soft i2c error -> RD addr 0x%02X nack", i2c_dev->dev_addr);
        return SOFT_I2C_ERR_ADDR;
    }
 ...
 * @return int  0 on success | others on failed
 */
int soft_i2c_master_mem_read(soft_i2c_dev_t *i2c_dev, uint32_t mem_addr, uint16_t addr_len, uint8_t *rd_data,
                             uint16_t data_len) {
...
static int soft_i2c_master_send_byte(soft_i2c_dev_t *i2c_dev, uint8_t byte) {
    ...
    if (soft_i2c_master_wait_ack(i2c_dev)) {
        soft_i2c_master_stop(i2c_dev);
        SOFT_I2C_ERROR("soft i2c error -> No ACK");
        return SOFT_I2C_ERR_NACK;
    }
    return SOFT_I2C_OK;
}
