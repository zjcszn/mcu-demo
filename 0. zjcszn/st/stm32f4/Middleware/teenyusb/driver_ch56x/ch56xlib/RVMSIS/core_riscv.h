...
RV_STATIC_INLINE uint32_t SysTick_Config(UINT64 cookie_ticks) {
    if ((cookie_ticks - 1) > SysTick_LOAD_RELOAD_Msk) return (1);      /* Reload value impossible */

    SysTick->CMP = cookie_ticks - 1;                                  /* set reload register */
