...
int main(int argc, char *argv[]) {
    ...
    fprintf(data_file,
            "#ifndef FS_FILE_FLAGS_HEADER_INCLUDED" NEWLINE "#define FS_FILE_FLAGS_HEADER_INCLUDED 1" NEWLINE "#endif" NEWLINE);
...
#if ALIGN_PAYLOAD
    fprintf(data_file,
            "#if FSDATA_FILE_ALIGNMENT==2" NEWLINE "#include \"fsdata_alignment.h\"" NEWLINE "#endif" NEWLINE);
#endif
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename) {
    ...
    fprintf(data_file,
            "static const " PAYLOAD_ALIGN_TYPE " dummy_align_%s = %d;" NEWLINE,
            varname,
    ...
    if (includeHttpHeader) {
        file_write_http_header(data_file,
                               filename,
                               file_size,
                               &http_hdr_len,
                               &http_hdr_chksum,
