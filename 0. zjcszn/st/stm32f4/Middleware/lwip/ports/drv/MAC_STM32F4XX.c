...
#define NUM_TX_BUF          4U          /* 0x0C00 for Tx (2*1536=3K)          */
...
static int32_t Control(uint32_t control, uint32_t arg) {
    ...
    switch (control) {
        case ARM_ETH_MAC_CONFIGURE:
            ...
            dmaomr = ETH->DMAOMR & ~(ETH_DMAOMR_RSF | ETH_DMAOMR_TSF | ETH_DMAOMR_DTCEFD | ETH_DMAOMR_FEF);
            
            /* All frames except runt error frames are forwarded to the DMA. */
            if (arg & ARM_ETH_MAC_FORWARD_ERROR_FRAME) {
                dmaomr |= ETH_DMAOMR_FEF;
            }
            
            /* Don't drop frames that only have errors detected by the receive checksum offload engine*/
            if (arg & ARM_ETH_MAC_DISABLE_DROP_ERROR_FRAME) {
