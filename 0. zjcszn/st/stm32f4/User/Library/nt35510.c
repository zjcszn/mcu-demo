...
#if       NT35510_USE_LOG
#include  <stdio.h>
#define   NT35510_LOG(...)            log_i(__VA_ARGS__)
#else
#define   NT35510_LOG(...)            ((void)0U)
#endif

#define NT35510_Delay(ms)             vTaskDelay(pdMS_TO_TICKS(ms))
 ...
 */
static inline void NT35510_WriteCmd(uint16_t _cmd) {
    NT35510->REG = _cmd;
}
 ...
 */
static inline void NT35510_WriteData(uint16_t _data) {
    NT35510->RAM = _data;
}
 ...
 */
static inline void NT35510_WriteReg(uint16_t reg, uint16_t reg_val) {
    NT35510->REG = reg;
    NT35510->RAM = reg_val;
}
 ...
 */
static inline uint16_t NT35510_ReadReg(uint16_t reg) {
    NT35510->REG = reg;
    return NT35510->RAM;
}
...
static void NT35510_WriteMultiData(uint16_t *buffer, uint32_t length) {
    lcd_fsmc_dma_write(buffer, length);
}
...
void NT35510_Init(NT35510_DispDirEnum disp_dir) {
    ...
    NT35510_LOG("lcd controller ID: %4x", read_id);
    if (read_id != 0x8000) {
        NT35510_LOG("lcd controller manufacture ID error!");
        return;
    }
...
void NT35510_SetScanDirection(NT35510_ScanDirEnum scan_dir) {
    ...
    NT35510_SetWindow(0, 0, nt35510_params.width, nt35510_params.height);
    NT35510_LOG("lcd display width  set: %u", nt35510_params.width);
    NT35510_LOG("lcd display height set: %u", nt35510_params.height);
}
...
void NT35510_DrawLine(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye, uint16_t color) {
    ...
    if (xs == xe)
        return NT35510_DrawVLine(xs, MIN(ys, ye), ABS((int)ys - (int)ye), color);
    if (ys == ye)
        return NT35510_DrawHLine(MIN(xs, xe), ys, ABS((int)xs - (int)xe), color);
