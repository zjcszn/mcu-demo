 ...
 */
static boot_params_t boot_params __attribute__((section(".bss.NoInit")));  /* this is demo for armcc v6 */



static uint16_t boot_params_calc_checksum(void);

static inline bool boot_params_is_valid(void);
...
bool boot_params_read(uint32_t idx, uint32_t *value) {
    ...
    log_d("boot params read  %s, [%2u]: %02X, checksum: %04X", result ? "success" : " failed", idx, *value,
          boot_params.checksum);
...
bool boot_params_write(uint32_t idx, uint32_t value) {
    ...
    log_d("boot params write %s, [%2u]: %02X, checksum: %04X", result ? "success" : " failed", idx, value,
          boot_params.checksum);
 ...
 */
static inline bool boot_params_is_valid(void) {
    if ((boot_params.magic_word == BOOT_PARAMS_MAGIC_WORD) && (boot_params.checksum == boot_params_calc_checksum())) {
        return true;
    }
    return false;
}
