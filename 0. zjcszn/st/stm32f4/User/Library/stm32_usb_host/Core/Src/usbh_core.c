...
USBH_StatusTypeDef USBH_Init(USBH_HandleTypeDef *phost,
    ...
    static uint8_t staticQueueStorageArea[MSGQUEUE_OBJECTS * sizeof(phost->os_msg)];
    static StaticQueue_t staticQueueStructure;
    phost->os_event = xQueueCreateStatic(MSGQUEUE_OBJECTS,
                                         sizeof(phost->os_msg),
                                         staticQueueStorageArea,
                                         &staticQueueStructure);
    
    /* Create USB Host Task */
    xTaskCreate(USBH_Process_OS,
                "usb host task",
                USBH_PROCESS_STACK_SIZE / sizeof(StackType_t),
                phost,
                USBH_PROCESS_PRIO,
                &(phost->thread));
    
    configASSERT(phost->thread != NULL);
...
USBH_StatusTypeDef USBH_DeInit(USBH_HandleTypeDef *phost) {
    ...
    vTaskDelete(phost->thread);
    vQueueDelete(phost->os_event);
...
USBH_StatusTypeDef USBH_ReEnumerate(USBH_HandleTypeDef *phost) {
    ...
    USBH_POST_EVENT_MESSAGE(phost->os_event, &(phost->os_msg));
#endif
...
static void USBH_Process_OS(void *argument) {
    ...
    for (;;) {
        status = xQueueReceive(((USBH_HandleTypeDef *)argument)->os_event,
                               &(((USBH_HandleTypeDef *)argument)->os_msg), portMAX_DELAY);
        if (status == pdTRUE) {
