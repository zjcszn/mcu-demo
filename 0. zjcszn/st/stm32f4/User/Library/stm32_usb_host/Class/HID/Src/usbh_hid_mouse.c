...
static USBH_StatusTypeDef USBH_HID_MouseDecode(USBH_HandleTypeDef *phost) {
    ...
    if (USBH_HID_FifoRead(&HID_Handle->fifo, &mouse_report_data, HID_Handle->length) == HID_Handle->length) {
        /*Decode report */
        mouse_info.x = (int16_t)HID_ReadItem((HID_Report_ItemTypedef *)&prop_x, 0U);
        mouse_info.y = (int16_t)HID_ReadItem((HID_Report_ItemTypedef *)&prop_y, 0U);
        mouse_info.wheel = (int8_t)HID_ReadItem((HID_Report_ItemTypedef *)&prop_wheel, 0U);
